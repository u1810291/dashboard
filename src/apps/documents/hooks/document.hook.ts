...
import { getPhotosOrientation, PhotosOrientations, VerificationDocument } from 'models/Document.model';
import { useEffect, useState } from 'react';
import { useIntl } from 'react-intl';
import { useSelector } from 'react-redux';
import { selectCountriesList } from 'state/countries/countries.selectors';

export function useDocumentTitle(document: VerificationDocument) {
  const intl = useIntl();
...
export function usePhotosOrientation(document: VerificationDocument) {
