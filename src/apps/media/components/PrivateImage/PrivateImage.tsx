...
export function PrivateImage({ src, alt, ...props }: ImgHTMLAttributes<HTMLImageElement>) {
  ...
  const updateImage = useCallback((chunks: any[], receivedLength: number) => {
    const blob = collectMediaChunks(chunks, receivedLength);
      ...
      try {
        ...
        while (true) {
          ...
          if (capacityForRender / contentLength > 0.1) {
            const objectUrl = updateImage(chunks, receivedLength);
            updateLastObjectUrl(objectUrl);
            capacityForRender = 0;
          }
        }

        const finalObjectUrl = updateImage(chunks, receivedLength);
