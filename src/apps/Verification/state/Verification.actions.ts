...
export const verificationListLoad = (identityId: string) => async (dispatch: Dispatch) => {
  dispatch({ type: VerificationActionTypes.VERIFICATIONS_PASSED_REQUEST });
  try {
    const { data }: { data: PassedVerificationsResponse[] } = await client.getVerificationList(identityId);
...
export const verificationLoad = (verificationId: string) => async (dispatch) => {
  dispatch({ type: VerificationActionTypes.VERIFICATION_REQUEST });
  try {
    const { data }: { data: VerificationResponse } = await client.getVerification(verificationId);
