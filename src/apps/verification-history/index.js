...
function getDoсumentTypes(facematchScore) {
  return uniq(flatten(facematchScore).filter(t => AVAILABLE_DOCUMENT_TYPES.includes(t)))
}
...
const tableColumns = [
  {
    ...
    content: ({ fullName }) => <VerificationFullNameLabel>{fullName}</VerificationFullNameLabel>,
  },
  {
    size: 4,
    label: <FormattedMessage id="identities.fields.documentTypes"/>,
    content: identity => <DocumentTypesLabel types={getDoсumentTypes(identity.facematchScore)} />,
  },
  {
    size: 2,
